...
This component lives under `examples/tools/mdx-validate/` in the dpm-tools repo.
...
`<os>/<arch>`. The commands below use dpm's `publish` and `tags` subcommands;
if either is missing, update to a recent [dpm release](https://github.com/digital-asset/dpm/releases).
...
> The older `override-components:` key still works but is **deprecated** —
> prefer `components:`.
